...
def parse_godot_scene(filepath):
    ...
    for match in ext_res_pattern.finditer(content):
        path, res_id = match.groups(); ext_resources[res_id] = os.path.basename(path)
...
def export_godot_scene_trees(t, project_path, output_file, exclude_dirs):
    ...
    with codecs.open(output_path, 'w', 'utf-8') as outfile:
        outfile.write(f"{t.get('header_scene_tree_title')}: {os.path.basename(project_root)}\n" + "=" * 80 + "\n\n")
        for file_path in tqdm(sorted(tscn_files), desc=t.get('progress_bar_analyzing_scenes'), unit=" scene"):
            ...
            outfile.write("\n\n" + "=" * 80 + "\n\n")
    logging.info(t.get('info_scene_tree_complete', path=output_path))